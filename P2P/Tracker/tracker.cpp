...
void handle_ClientFunctions(int client_socket) 
{
    while (true) 
    {
        ...
        {
            handle_create_user(tokenise_commands, client_socket);
        }
        else if(tokenise_commands[0]=="login")
        {
            handle_login(tokenise_commands, client_socket);
        }
        else if(tokenise_commands[0]=="create_group")
        {
            handle_create_group(tokenise_commands, client_socket);
        }
        else if(tokenise_commands[0]=="join_group")
        {
            handle_join_group(tokenise_commands, client_socket);
        }
        else if(tokenise_commands[0]=="list_requests")
        {
            handle_list_requests(tokenise_commands, client_socket);
        }
        else if(tokenise_commands[0]=="list_groups")
        {
            handle_list_groups(tokenise_commands, client_socket);
        }
        else if(tokenise_commands[0]=="accept_request")
        {
            handle_accept_request(tokenise_commands, client_socket);
        }
        else if(tokenise_commands[0]=="leave_group")
        {
            handle_leave_group(tokenise_commands, client_socket);
        }
        else if (tokenise_commands[0] == "logout") 
        {
            handle_logout(tokenise_commands, client_socket);
        }
        else if(tokenise_commands[0] == "upload_file")
        {
            handle_upload_file(tokenise_commands, client_socket);
        }
        else if(tokenise_commands[0]=="download_file")
        {
            handle_download_file(tokenise_commands, client_socket);
        }
        else if(tokenise_commands[0]=="stop_share")
        {
            handle_stop_share(tokenise_commands, client_socket);
        }
        else
        {
